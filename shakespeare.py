...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt", type=str, default="text_ckpt.pth")
    ...
    if args.sample:
        diff_model.load_state_dict(torch.load(args.ckpt, map_location=device))
        z = sample_diffusion_embeddings(diff_model, dim, device, args.n, args.seq_len)
        Path("samples").mkdir(exist_ok=True)
        torch.save(z.cpu(), "samples/diff_embeds.pt")
        print("✔ Saved raw embeddings to samples/diff_embeds.pt")
